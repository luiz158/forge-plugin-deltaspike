 ...
 * <p>predicates included here all starts with "with", for example
 * {@link ReflectionUtils#withAnnotation(java.lang.annotation.Annotation)}, {@link ReflectionUtils#withModifier(int)},
 * {@link ReflectionUtils#withParametersAssignableFrom(Class[])}
...
public final class ReflectionUtils {
    ...
    public static final List<String> primitiveNames = Lists.newArrayList("boolean", "char", "byte", "short", "int",
            "long", "float", "double", "void");

    @SuppressWarnings({"unchecked"})
    public static final List<Class> primitiveTypes = Lists.<Class>newArrayList(boolean.class, char.class, byte.class,
            short.class, int.class, long.class, float.class, double.class, void.class);

    public static final List<String> primitiveDescriptors = Lists.newArrayList("Z", "C", "B", "S", "I", "J", "F",
            "D", "V");
    ...
    public static Set<Class<?>> getAllSuperTypes(final Iterable<Class<?>> types,
                                                 Predicate<? super Class<?>> predicate) {
        Set<Class<?>> result = Sets.newHashSet();
        for (Class<?> type : types) result.addAll(getAllSuperTypes(type, predicate));
        return result;
    }
    ...
    public static Set<Field> getAllFields(final Iterable<Class<?>> types, Predicate<? super Field> predicate) {
        Set<Field> result = Sets.newHashSet();
        for (Class<?> type : types) result.addAll(getAllFields(type, predicate));
        return result;
    }
    ...
    public static Set<Method> getAllMethods(final Iterable<Class<?>> types, Predicate<? super Method> predicate) {
        Set<Method> result = Sets.newHashSet();
        for (Class<?> type : types) result.addAll(getAllMethods(type, predicate));
     ...
     */
    public static <T extends AnnotatedElement> Set<T> getAll(final Iterable<T> elements,
     ...
     */
    public static <T extends AnnotatedElement> Predicate<T> withAnnotation(final Class<? extends Annotation>
                                                                                   annotation) {
        return new Predicate<T>() {
            public boolean apply(T input) {
        ...
        if (annotation2 != null && annotation1.annotationType() == annotation2.annotationType()) {
            for (Method method : annotation1.annotationType().getDeclaredMethods()) {
                try {
                    ...
                    throw new JavaMetaDataException(String.format("could not invoke method %s on annotation %s",
                            method.getName(), annotation1.annotationType()), e);
                }
            }
            return true;
        }
     ...
     */
    public static <T extends AnnotatedElement> Set<T> getMatchingAnnotations(final Set<T> annotatedElements,
     ...
     * <p>if optional {@link ClassLoader}s are not specified, then both {@link org.apache.deltaspike.forge.helper
     * .overview.util
     * .ClasspathHelper#contextClassLoader()} and {@link org.apache.deltaspike.forge.helper.overview.util
